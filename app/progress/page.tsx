...
function dedupeWeeks(weeks: WeekListEntry[]): WeekListEntry[] {
  const sortedByUpdate = [...weeks].sort((a, b) => {
    return new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime();
  });

  const map = new Map<string, WeekListEntry>();

  for (const week of sortedByUpdate) {
    const existing = map.get(week.weekOf);
    if (!existing) {
      map.set(week.weekOf, week);
      continue;
    }

    if (existing.status !== "active" && week.status === "active") {
      map.set(week.weekOf, week);
    }
  }
...
function buildProgressData(weeks: WeekListEntry[]): ProgressData {
  const normalizedWeeks = dedupeWeeks(weeks);
